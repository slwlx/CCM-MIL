...
def collate_MIL_survival(batch):
    ...
    coods = torch.cat([item[5] for item in batch], dim=0)
    #img_128 = torch.cat([item[6] for item in batch], dim=0)
    #coods_128 = torch.cat([item[7] for item in batch], dim=0)
    if isinstance(img[0], torch.Tensor):
        # 旧逻辑：单尺度
        img = torch.cat(img, dim=0)
    elif isinstance(img[0], dict):
        # 多尺度：保留 list of dict，不 cat
        pass  # 不修改 img，保持为 list of dict
    elif isinstance(img[0], (tuple, list)):
        # 多尺度：保留 list of tuple
        pass  # 不 cat
    else:
        raise ValueError(f"Unsupported img type: {type(img[0])}")


    return [img, omic, label, event_time, c, coods, ]
